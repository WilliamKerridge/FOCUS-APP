...
export default function QuickCaptureFAB({ onCapture, showMadeTo, placeholder = 'Add a task' }: Props) {
  ...
  return (
    <>
      ...
      {/* Sheet */}
      {open && (
        <div className="fixed bottom-20 right-4 z-50 w-80 rounded-2xl bg-background border border-border shadow-xl p-4 space-y-3">
          {saved ? (
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className="fixed bottom-6 right-4 z-50 w-12 h-12 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center text-2xl font-light cursor-pointer motion-safe:active:scale-95 motion-safe:transition-transform"
